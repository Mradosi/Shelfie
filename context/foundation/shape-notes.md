...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: pain categories
      decision: workflow friction, decision paralysis, missing capability, coordination overhead
    - topic: product insight
      decision: context-aware guidance rooted in owned products and reversible routine overlays
    - topic: primary persona scope
      decision: single named user including the user themself for MVP shaping
    - topic: access model
      decision: authenticated email login with a flat user model; each signed-in user only sees and manages their own data
    - topic: MVP timeline choice
      decision: user accepted a six-week after-hours MVP instead of scoping down
    - topic: temporary routine scope
      decision: temporary routine handling is optional for MVP; if included, it should be a separate AI-generated short-term routine rather than overlay logic
    - topic: business logic rule
      decision: Shelfie analyzes skin context, owned products, routine structure, and feedback to generate and adapt personalized skincare guidance while helping avoid problematic combinations or overuse
    - topic: product type
      decision: website or web app
    - topic: target scale
      decision: just the user or a handful of users for the initial live version
    - topic: scaling note
      decision: no meaningful change to the domain rule at 100x scale
    - topic: hard deadline
...
Acknowledged on 2026-05-22: 6-week MVP requires sustained dedication; user accepted.
...
- **When** they add cosmetic products to their shelf using manual entry or image-based AI adding from uploaded or camera photos, confirm the recognized product information, and request skincare guidance
...
- FR-004: User can add products by manual entry or image-based AI adding from uploaded or camera photos. Priority: must-have
  > Socrates: Counter-argument considered: supporting four input methods at once broadens scope too early. Resolution: narrowed the MVP to manual entry plus image-based AI adding; product name lookup can be deferred if needed.
...
- FR-009: User can view a structured morning/evening or weekly routine. Priority: must-have
  > Socrates: Counter-argument considered: none. Resolution: kept as written because the guidance must be presented as a structured routine the user can follow.
...
- FR-011: User can receive lightweight warnings about potentially problematic ingredient combinations or overuse, especially when manually changing routines or combining owned products. Priority: must-have
  > Socrates: Counter-argument considered: a strict conflict-validation engine may be too heavy for the MVP. Resolution: kept the warning system in MVP, but narrowed it to soft guidance rather than blockers; baseline AI-generated routines should avoid obvious conflicts, and part of conflict detection may use deterministic ingredient-group rules with AI focused on explanations.
...
- No advanced barcode scanning or OCR pipelines, because the MVP keeps product adding to manual entry plus lightweight image-based AI assistance.
...
## Forward: optional enhancements

- Temporary short-term routines for irritation, cosmetic procedures, or similar situations remain a post-core optional enhancement rather than a required MVP capability.

## Quality cross-check

All soft-gate checks passed: access control, business logic, project artifacts, timeline acknowledgment, and non-goals are present.

## Forward: tech-stack

User noted a likely mobile-first web app direction with PWA support, and previously mentioned React as a possible frontend direction for downstream stack selection.
