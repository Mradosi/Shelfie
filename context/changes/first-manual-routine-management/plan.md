...
- [ ] 1.3 Role-bearing routine entries round-trip through the domain layer without losing role or array order
- [ ] 1.4 Full reset clears `user_routine_configs.schedule` without removing owned shelf items

### Phase 2: Protected manual routine surface

#### Automated

- [x] 2.1 Production build passes with the new protected routine route and editor surface

#### Manual

- [x] 2.2 User with owned shelf items can assign roles inline, build a base `AM/PM` routine, reorder entries, save it, refresh the page, and see the same state loaded back
- [x] 2.3 User with zero or very few products sees soft guidance and CTA to `/products/intake`, but the route itself remains accessible

### Phase 3: Flow integration and lifecycle polish

#### Automated

- [ ] 3.1 Final repository verification passes after the full slice lands

#### Manual

- [ ] 3.2 After product intake, the user can move directly into routine creation from the success surface and later re-enter routine editing from the main navigation
- [ ] 3.3 After deleting the routine, the user sees a clean empty state ready to rebuild from scratch, with shelf products still intact
