...
function assessmentInput() {
  ...
  return { fixtures, shelf: createRoutineAiShelfInputs(fixtures.shelfCatalog, fixtures.products, fixtures.interpretations) };
}

describe("parseRoutineAiAssessment", () => {
  it("accepts a complete audit for every product pair", () => {
    const { fixtures, shelf } = assessmentInput();
    expect(parseRoutineAiAssessment(fixtures.validAssessment, fixtures.currentDraft, shelf)).toEqual(fixtures.validAssessment);
  });

  it("rejects a missing compatibility pair before persistence", () => {
    const { fixtures, shelf } = assessmentInput();
    expect(() => parseRoutineAiAssessment({ ...fixtures.validAssessment, compatibilityAudit: [] }, fixtures.currentDraft, shelf)).toThrow(
      "Ocena rutyny musi zawierać werdykt dla każdej pary produktów.",
    );
  });
