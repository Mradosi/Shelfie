...
set
  stored_image_url = coalesce(
    stored_image_url,
    case
      when image_url like '%/storage/v1/object/public/product-images/%' then image_url
      else null
    end
  ),
  source_image_url = coalesce(
    source_image_url,
    case
      when image_url like '%/storage/v1/object/public/product-images/%' then null
      else image_url
    end
  )
where image_url is not null;

drop function if exists public.upsert_confirmed_product(text, text, text, text, text[], text, text, text, text, timestamptz);
